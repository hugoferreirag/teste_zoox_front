    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
            ...
            <v-form ref="form" lazy-validation>
              <v-row justify="center">
                <v-col md="12" align="center">
                  <v-alert v-if="createSuccess" dark outlined color="green">Estado inserido com sucesso</v-alert>
                  <v-alert v-if="error" dark outlined color="red">{{error}}</v-alert>
                </v-col>
              </v-row>
              <v-row>
                <v-col cols="12" sm="6" md="8">
                  <v-text-field :rules="nameRules" v-model="state.name" label="Nome do Estado" hint="example of helper text only on focus"></v-text-field>
                </v-col>
                <v-col cols="12" sm="6" md="4">
                  <v-text-field :rules="initialsRules" v-model="state.initials" label="Abreviação" required></v-text-field>
                </v-col>
              </v-row>
            </v-form>
          </v-container>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="createState">Salvar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-row>
  ...
  methods: {
    ...
    async createState () {
      if (this.$refs.form.validate()) {
        await this.newState(this.state)
